	...
	def __init__(self, char_color, x, y, scale, speed, ammo, grenades, health):
		...
		for animation in animation_types:
			...
			for i in range(num_of_frames):
				...
				img = pygame.transform.scale(img, (scale * 0.8, scale))
			...
			for x, tile in enumerate(row):
				if tile >= 0:
					...
					if tile >= 0 and tile <= 8:
						...
						player = Soldier(player_color, x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, 5, 20, 5, 100)
						health_bar = HealthBar(10, 10, player.health, player.health)
					elif tile == 16: # Create enemies
						enemy = Soldier('red', x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, 2, 20, 0, 100)
...
while run:
	...
	if not start_game:
		...
		# Select which player color to use
		for color, player_btn in player_color_buttons_dict.items():
			player_btn.update()
			# Check if player_btn is clicked
			if player_btn.draw():
				# Reset all other player_btns except the on clicked
				for player_btn_2 in player_color_buttons_dict.values():
					if player_btn_2 != player_btn:
						player_btn_2.mouse_action = False
				# Paint the player_btn and select player_color
